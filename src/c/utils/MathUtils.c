...
// ---------------------------------------------------------------------------
// Credit to JavierRizzoA, author of the excellent Mercury watchface, for some of the Utlities below
// https://github.com/JavierRizzoA/Pebble-Mercury
// https://apps.rebble.io/en_US/application/67cdf003b7a023034a3395bc
// ---------------------------------------------------------------------------
...
GPoint polar_to_point(int angle, int distance) {
  int x = (cos_lookup(DEG_TO_TRIGANGLE(angle)) * distance) / TRIG_MAX_RATIO;
  int y = (sin_lookup(DEG_TO_TRIGANGLE(angle)) * distance) / TRIG_MAX_RATIO;
  return GPoint(x, y);
}
